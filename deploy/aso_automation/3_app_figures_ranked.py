...
def save_intermediate(final_results, product, iteration):
    filename = f"keywords_{product}_{iteration}_ranked.json"
    try:
...
def call_app_figures_api(country, product_ids):
    ...
    for item in data.get("results", []):
        rank = item["products"][product_ids[0]]['position']
        if rank > 110:
            continue
        keywords.append({
...
def main():
    ...
    # Use our own product IDs instead of competitor products
    own_products = {
        "play_market": [product_desc[product]['product_id']['play_market']],
        "app_store": [product_desc[product]['product_id']['app_store']]
    }
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        future_to_task = {}
        for store_type in own_products:
            ...
            for country in app_figures_languages:
                if country in country_to_locales:
                    future = executor.submit(call_app_figures_api, country, own_products[store_type])
    ...
    output_filename = f"keywords_{product}_{iteration}_ranked.json"
    try:
