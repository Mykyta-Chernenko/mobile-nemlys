...
import { SchedulableNotificationTriggerInput } from 'expo-notifications/src/Notifications.types';
...
export default function ({
  route,
  navigation,
}: NativeStackScreenProps<MainStackParamList, 'V2Home'>) {
  ...
  async function scheduleFirstDateNotification() {
    ...
    const triggerSeconds = [60 * 60 * 2, 60 * 60 * 12];
    const notifications = (
      _.zip(notificationOrder, triggerSeconds) as [NOTIFICATION_SUBTYPE, number][]
    ).map(([subtype, seconds]) => ({
      screen: 'Home',
  ...
  async function getIsOnboarded() {
    ...
    if (!userProfileData.data.onboarding_finished) {
      void navigation.navigate('YourName', { fromSettings: false });
      return;
    } else {
      const coupleId = userProfileData.data.couple_id;
      setCoupleId(coupleId);
      //  TODO not showing interview when users come back second day now
