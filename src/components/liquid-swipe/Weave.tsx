...
import Animated, { SharedValue, runOnJS, useDerivedValue } from 'react-native-reanimated';
...
interface WeaveProps {
  centerYp: SharedValue<number>;
  horRadiusp: SharedValue<number>;
  vertRadiusp: SharedValue<number>;
  sideWidthp: SharedValue<number>;
  children: ReactNode;
}

export default ({ centerYp, horRadiusp, vertRadiusp, sideWidthp, children }: WeaveProps) => {
  const { width, height } = Dimensions.get('window');
  const [commands, setCommands] = useState<string[]>(['']);
  const AnimatedPath = Animated.createAnimatedComponent(Path);
  useDerivedValue(() => {
    const centerY = centerYp.value;
    const horRadius = horRadiusp.value;
    const vertRadius = vertRadiusp.value;
    const sideWidth = sideWidthp.value;
    const commands = [];
    const curveStartY = centerY + vertRadius;
    const maskWidth = width - sideWidth;
    const sub = (a, b) => a - b;
    const multiply = (a, b) => a * b;
    moveTo(commands, maskWidth - sideWidth, 0);
    lineTo(commands, 0, 0);
    ...
    close(commands);
    runOnJS(setCommands)(commands);
  }, []);

  const d = commands.reduce((acc, c) => acc + c);
  const maskElement = (
    <MaskedViewIOS
